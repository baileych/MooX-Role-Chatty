...
sub _trigger_verbose {
  ...
  if ($my_adapter) {
    Log::Any::Adapter->set({ category => $category }, 'Carp', no_trace => 1,
...
has 'logger' => ( is => 'rw', lazy => 1,
		  isa => Type::Tiny::Duck->new(methods => [ qw/info warn/ ]),
		  builder => '_build_logger', clearer => 'clear_logger',
		  trigger => 1);
...
sub _build_logger {
  ...
  Log::Any::Adapter->set({ category => $category }, 'Carp', no_trace => 1,
			 log_level => $self->_verbose_to_log_level);
  $self->_internal_logger(Log::Any->get_logger(category => $category,
